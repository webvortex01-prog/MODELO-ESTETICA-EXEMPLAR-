...
export default function ProtocolSimulator({ procedures, onSelectProcedure }: ProtocolSimulatorProps) {
  ...
  return (
    <motion.div 
      ...
      className="bg-gradient-to-b from-[#101216] via-[#0A0A0A] to-[#050505] border border-[#D4AF37]/30 rounded-3xl p-8 md:p-14 shadow-[0_0_60px_rgba(212,175,55,0.1)] relative overflow-hidden"
    >
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <div className="text-center mb-12 space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30">
            ...
            <span className="font-mono text-[#D4AF37] text-[10px] uppercase tracking-[0.3em]">Inteligência Estética Lumière</span>
          </div>
          ...
          <p className="font-mono text-[11px] text-stone-400 uppercase tracking-widest max-w-lg mx-auto leading-relaxed">
        ...
        {step === 1 && (
          <motion.div 
            ...
            className="space-y-8"
          >
            <p className="font-mono text-xs uppercase tracking-widest text-[#D4AF37] text-center font-semibold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                >
                  ...
                  <p className="font-mono text-[10px] text-stone-400 uppercase tracking-widest">{item.desc}</p>
                </motion.div>
              ))}
            </div>
            <div className="text-center pt-4">
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                className="px-10 py-4.5 rounded-full bg-gradient-to-r from-[#D4AF37] to-[#F3E5AB] text-black font-mono text-[10px] uppercase tracking-[0.3em] font-bold shadow-[0_0_25px_rgba(212,175,55,0.3)] hover:shadow-[0_0_35px_rgba(212,175,55,0.6)] transition-all inline-flex items-center gap-3 cursor-pointer"
              >
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                >
                  <p className="font-mono text-xs uppercase tracking-widest font-medium">{item.label}</p>
                </motion.div>
              ))}
            </div>
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
              <button 
                onClick={() => setStep(1)} 
                className="px-8 py-3.5 rounded-full border border-stone-800 text-stone-400 font-mono text-[10px] uppercase tracking-widest hover:text-white hover:border-stone-600 transition-all cursor-pointer"
              >
                Voltar
              </button>
              <motion.button 
        ...
        {step === 3 && result && (
          <motion.div 
            ...
            className="space-y-8 bg-gradient-to-b from-[#141619] to-[#0C0D0F] p-8 md:p-10 rounded-3xl border border-[#D4AF37]/50 shadow-2xl"
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6">
              <div>
                ...
                <h4 className="font-serif text-3xl md:text-4xl text-white mt-1">{result.name}</h4>
              </div>
              <div className="md:text-right">
                <span className="font-mono text-[9px] text-stone-500 uppercase tracking-widest block">Investimento Estimado</span>
                <span className="font-serif text-3xl text-[#D4AF37]">R$ {result.price.toLocaleString('pt-BR')}</span>
              </div>
            </div>

            <p className="font-mono text-xs text-stone-300 uppercase tracking-wider leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-white/10">
              <button 
                ...
                className="text-stone-400 font-mono text-[10px] uppercase tracking-widest hover:text-white inline-flex items-center gap-2 transition-colors cursor-pointer"
              >
